...
def build_features_table(clients_dict, clients_info_path=None):
    """Собирает фичи по всем клиентам и, если указан путь к clients.csv, мёрджит с базовой инфой."""
    rows = []
    ...
    feat = pd.DataFrame(rows).set_index('client_code').fillna(0).reset_index()

    # Если указан файл clients.csv — подтянем его данные
    if clients_info_path and os.path.exists(clients_info_path):
        info_df = pd.read_csv(clients_info_path)
        # приведение к нижнему регистру для совместимости с benefit_calculator
        if 'avg_monthly_balance_KZT' in info_df.columns:
            info_df = info_df.rename(columns={'avg_monthly_balance_KZT': 'avg_monthly_balance_kzt'})
        feat = feat.merge(info_df, on='client_code', how='left')
